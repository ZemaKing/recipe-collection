...
function DeleteSubcategoryDialog({
  ...
  return (
    <Dialog open={subcategoryName !== null} onOpenChange={(open) => !isDeleting && onOpenChange(open)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('admin.subcategories.deleteTitle')}</DialogTitle>
          <DialogDescription>{t('admin.subcategories.deleteDescription', { name: subcategoryName })}</DialogDescription>
        </DialogHeader>
