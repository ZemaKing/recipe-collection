...
function DeleteIngredientDialog({
  ...
  return (
    <Dialog open={ingredientName !== null} onOpenChange={(open) => !isDeleting && onOpenChange(open)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('admin.ingredients.deleteTitle')}</DialogTitle>
          <DialogDescription>{t('admin.ingredients.deleteDescription', { name: ingredientName })}</DialogDescription>
        </DialogHeader>
